 ...
 * @version $Id: Activator.java 580 2013-01-13 21:45:30Z LukKnapen $
 *
 */
public class Activator extends be.uhasselt.imob.feathers2.services.impl.abstractService.Activator {

	private CommonsService commonsSrv = null;
	...
	private synchronized void reRegister()
	{
		...
		if (commonsSrv != null) commonsSrv.cleanup();
				...
				true)
		{
			try
			{
				commonsSrv = new CommonsService(config);
				logger.debug("Created CommonsService");
				if (commonsSrv == null) logger.error("Activator.reRegister() : commonsSrv == null");
            ...
            // The key is the type used in the specified method (in order to generate an unambiguous mechanism to find the method to be called (using reflection)).
				servicesProvided.add(context.registerService("be.uhasselt.imob.feathers2.services.commonsService.ICommonsService", commonsSrv, opts));
				logger.debug("Registered CommonsService");
			}
			catch (Throwable e)
			{
